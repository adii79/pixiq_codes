...
//	  char txBuf[] = "hi";
//
//	      CC1101_Communicate(true, txBuf);
////	      HAL_UART_Transmit(&huart9, (uint8_t *)"TX: hi\n", 7, 10);
//
//	      HAL_Delay(500);   // critical


	  char rxBuf[64];

	    if (CC1101_Communicate(false, rxBuf))
	    {
	        HAL_UART_Transmit(&huart9, (uint8_t *)"RX: ", 4, 10);
	        HAL_UART_Transmit(&huart9, (uint8_t *)rxBuf, strlen(rxBuf), 10);
	        HAL_UART_Transmit(&huart9, (uint8_t *)"\r\n", 2, 10);
	    }
	    else
	    {
	        HAL_UART_Transmit(&huart9, (uint8_t *)"No data\r\n", 9, 10);
	    }

	    HAL_Delay(200);  // avoid tight loop
