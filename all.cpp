...
// #define DATA_PIN       4
// #define PIN_ENABLE     8
// #define HUE_B          3   // Button: cycles Hue
// #define SATURATION_B   0   // Button: cycles Saturation
// #define INTENSITY_B    1   // Button: cycles Intensity (Value)
...
// // ── HSV color state (FastLED range: 0–255 each) ─────────
// uint8_t hue_selected        = 0;
// uint8_t saturation_selected = 255;  // Full saturation by default
// uint8_t intensity_selected  = 200;  // ~78% brightness by default

// // ── Debounce config ──────────────────────────────────────
// #define DEBOUNCE_MS 200
// unsigned long lastHuePress  = 0;
// unsigned long lastSatPress  = 0;
// unsigned long lastIntPress  = 0;

// void setup() {
//   Serial.begin(115200);

//   pinMode(HUE_B,       INPUT_PULLUP);  // LOW when pressed
//   pinMode(SATURATION_B,INPUT_PULLUP);
//   pinMode(INTENSITY_B, INPUT_PULLUP);

//   pinMode(PIN_ENABLE, OUTPUT);
//   digitalWrite(PIN_ENABLE, HIGH);

//   FastLED.addLeds<WS2812B, DATA_PIN, COLOR_ORDER>(leds, NUM_LEDS);
//   FastLED.setBrightness(255);  // Let CHSV value control brightness instead
// }

// void loop() {
//   unsigned long now = millis();

//   // ── HUE button: steps through 8 hue zones ───────────
//   if (digitalRead(HUE_B) == LOW && (now - lastHuePress) > DEBOUNCE_MS) {
//     hue_selected += 32;         // 256 / 32 = 8 distinct hue steps
//     lastHuePress = now;
//     Serial.print("Hue: "); Serial.println(hue_selected);
//   }

//   // ── SATURATION button: 4 levels (255 → 192 → 128 → 64 → 255) ──
//   if (digitalRead(SATURATION_B) == LOW && (now - lastSatPress) > DEBOUNCE_MS) {
//     saturation_selected -= 64;  // Wraps: 255→191→127→63→255
//     if (saturation_selected < 63) saturation_selected = 255;
//     lastSatPress = now;
//     Serial.print("Saturation: "); Serial.println(saturation_selected);
//   }

//   // ── INTENSITY button: 5 brightness levels ───────────
//   if (digitalRead(INTENSITY_B) == LOW && (now - lastIntPress) > DEBOUNCE_MS) {
//     intensity_selected += 50;   // Steps: 50 → 100 → 150 → 200 → 250 → wraps
//     if (intensity_selected > 250) intensity_selected = 50;
//     lastIntPress = now;
//     Serial.print("Intensity: "); Serial.println(intensity_selected);
//   }

//   // ── Apply color to all LEDs ──────────────────────────
//   CHSV hsv_color(hue_selected, saturation_selected, intensity_selected);
//   for (int i = 0; i < NUM_LEDS; i++) {
//     leds[i] = hsv_color;
//   }
//   FastLED.show();

//   delay(10);  // Small loop delay (debounce handles button timing)
// }